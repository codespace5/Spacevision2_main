...
class EIADataView(GenericAPIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(serializer.errors, status=HTTP_400_BAD_REQUEST)
class EIAUpdateView(GenericAPIView):
    serializer_class = CrudeOilSerializer
    queryset = CrudeOil.objects.all()
    ...
    def post(self, request):
        delete_data = request.data.get('date')
        "delted"
        print("delted_data:",delete_data)
        if delete_data is not None:
            # record_delete = CrudeOil.objects.filter(=delete_data)

            # print("filter datae is :", record_delete)
            # record_delete.delete()
            return Response({"status": "success", 'data': 'delted'}, status=HTTP_201_CREATED)
        else:
            return Response({"status": "failture", 'data': 'delete'}, status=HTTP_201_CREATED)
